...
def main():
    config = OmegaConf.load("query_attack.yaml")
    ...
    for batch_i, ((adv_images, _, path), (clean_images, _, _)) in enumerate(zip(data_loader, clean_data_loader)):
        ...
        tgt_text_features = get_texts_clip_features(batch_tgt_texts, clip_models, device)
        ...
        best_clip_scores = {}
        query_attack_results = np.zeros_like(batch_size)
        best_adv_images = np.zeros_like(adv_images)

        with torch.no_grad():
            victim_captions = generate_captions(config.unidif, adv_images, nnet, caption_decoder,autoencoder, clip_model, clip_model_img_preprocess)
        ...
        for step_idx in range(config.rgf_steps):
            ...
            perturbed_versions= torch.clamp(images_repeat + (sigma * query_noise), 0, 1)  # size = (num_query x batch_size, 3, 224, 224)
            ...
            with torch.no_grad():
                ...
                adv_text_features = get_texts_clip_features(unidiff_captions, clip_model, device)

                clip_scores = compute_clip_scores(unidiff_captions, batch_tgt_texts, clip_models, device)
                if best_clip_scores == {}:
                ...
                best_captions = np.where(is_better, unidiff_captions, best_captions)
                ...
                for model_name in clip_models.keys():
                    best_clip_scores[model_name] = np.where(is_better, clip_scores[model_name], best_clip_scores[model_name])
